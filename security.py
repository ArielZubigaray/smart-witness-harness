...
def validate_pkill_command(command_string: str) -> tuple[bool, str]:
    allowed_process_names = {"node", "npm", "npx", "vite", "next", "python", "python3", "uvicorn", "fastapi"}
    try:
...
async def bash_security_hook(input_data, tool_use_id=None, context=None):
    ...
    if not commands:
        return {"decision": "block", "reason": f"Could not parse command for security validation: {command}"}
    segments = split_command_segments(command)
    for cmd in commands:
        if cmd not in ALLOWED_COMMANDS:
            return {"decision": "block", "reason": f"Command '{cmd}' is not in the allowed commands list"}
